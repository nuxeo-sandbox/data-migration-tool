...
/**
 * A representation for an exported document which does not export the blob
 * itself.
 *
 */
public class ExportedDocumentNoBlobsImpl implements ExportedDocument {

    private static Log log = LogFactory.getLog(ExportedDocumentNoBlobsImpl.class);
    ...
    public ExportedDocumentNoBlobsImpl() {
    }
     ...
     *            paths
     */
    public ExportedDocumentNoBlobsImpl(DocumentModel doc, Path path, boolean inlineBlobs) throws IOException {
        id = doc.getId();
        if (path == null) {
            this.path = new Path("");
        } else {
    ...
    public ExportedDocumentNoBlobsImpl(DocumentModel doc) throws IOException {
        this(doc, false);
    }

    public ExportedDocumentNoBlobsImpl(DocumentModel doc, boolean inlineBlobs) throws IOException {
    ...
    protected void readDocumentSchemas(Element element, DocumentModel doc, boolean inlineBlobs) throws IOException {
        ...
        for (String schemaName : schemaNames) {
            ...
            try {
                ...
                log.error("error for uuid: " + doc.getId() + " error: " + e);
    ...
    protected final void readBlob(Element element, ComplexType ctype, Blob blob, boolean inlineBlobs)
        ...
        /*
         * Don't write blob infos... Method is final so I can't derive from this
         * class and just override only this method...
         * 
         * Element data = element.addElement(ExportConstants.BLOB_DATA); if
         * (inlineBlobs) { String content =
         * Base64.encodeBase64String(blob.getByteArray());
         * data.setText(content); } else { data.setText(blobPath);
