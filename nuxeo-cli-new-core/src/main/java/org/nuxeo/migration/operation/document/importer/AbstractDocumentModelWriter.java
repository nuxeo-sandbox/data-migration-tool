...
public abstract class AbstractDocumentModelWriter extends AbstractDocumentWriter {
            ...
            if (ACL.LOCAL_ACL.equals(element.attributeValue(ExportConstants.NAME_ATTR))) {
                ...
                if (size > 0) {
                    ...
                    for (Element el : entries) {
                        ...
                        ACE ace = ACE.builder(username, permission).isGranted(Boolean.parseBoolean(grant)).creator(
                                creator).begin(begin).end(end).build();
                        acl.add(ace);
                    }
                    acp.addACL(acl);
    ...
    protected static Class getFieldClass(Type fieldType) {
        ...
        // we do the recursion here and not in JavaTypes to avoid potential
        // impacts
    ...
    private static Object getElementData(ExportedDocument xdoc, Element element, Type type) {
        ...
        if (type.isSimpleType()) {
            ...
            if (TypeConstants.isContentType(ctype)) {
                ...
                String digest = element.elementTextTrim(ExportConstants.BLOB_DIGEST);
                ...
                if (blob == null) {
                    // maybe the blob is embedded in Base64
                    // encoded data
                    //
                    // we have exported without document
                    // so we import here also without a document
                    //
                    if (content != null) {
                        ...
                        try {
                            blob = blobManager.readBlob(blobInfo, repo);
                            blob.setDigest(digest);
                        } catch (IOException | RuntimeException e) {
                            log.error("Doc uuid: " + xdoc.getId() + " - No blob found for digest: " + digest + " " + e);
